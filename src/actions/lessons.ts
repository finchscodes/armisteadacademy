...
export async function reorderLessonAction(formData: FormData) {
  ...
  await db.update(lessons).set({ position: other.position }).where(eq(lessons.id, lessonId));
  await db.update(lessons).set({ position: siblings[index].position }).where(eq(lessons.id, other.id));
