...
import { jobLabel, type CharacterJob } from "@/lib/roles";
...
export function ArticleBoardPermissionCard({
  ...
  boardId: number;
  boardName: string;
  extraArticleJob: CharacterJob | null;
  granted: Granted[];
}) {
  ...
  return (
    <div className="bg-ink-900 border border-ink-700 rounded-lg p-4">
      ...
      <p className="text-xs text-ink-400 mb-3">
        Head Staff and up can always post here
        {extraArticleJob && <> — so can {jobLabel(extraArticleJob)}s</>}. Grant posting rights to
        anyone else below.
      </p>
