...
export const giftTriggerEnum = pgEnum("gift_trigger", ["birthday"]); // more triggers can be added later
...
/**
 * Admin-configured rules for automatic system gifts — e.g. "on a
 * character's birthday, send them a Spymaster's Watch." A trigger can
 * have several active rules at once (send more than one item), each
 * fired independently. Not tied to a sending character's inventory —
 * these are generated by the system, not transferred from a player. See
 * lib/automatic-gifts.ts.
 */
export const automaticGiftRules = pgTable("automatic_gift_rules", {
  id: serial("id").primaryKey(),
  trigger: giftTriggerEnum("trigger").notNull(),
  itemId: integer("item_id")
    .notNull()
    .references(() => items.id, { onDelete: "cascade" }),
  quantity: integer("quantity").notNull().default(1),
  // Shown to the recipient as who the gift is "from" — a label, not a
  // real sender character (the Spymaster job may be vacant or change
  // hands, so this isn't tied to whoever currently holds it).
  senderLabel: varchar("sender_label", { length: 100 }).notNull().default("The Spymaster"),
