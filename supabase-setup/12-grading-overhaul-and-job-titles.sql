-- Run this in Supabase's SQL Editor after 11-remove-pets.sql.
-- Adds:
--   - The multi-grader consensus grading system: 4 independent graders per
--     submission (grade_tier enum: Perfect/Excellent/Good/Needs
--     Improvement/Failing), replacing the old single-claimer 0-100 model.
--     Any submission that was mid-claim goes back to "open" and re-enters
--     the new queue — nothing is lost, just re-queued under the new system.
--   - Lesson reordering (position column) and the ability to delete a user
--     account without being blocked by lessons they created.
--   - Custom per-character job titles (e.g. multiple "Head Staff" leading
--     different teams) shown on the Job List.
