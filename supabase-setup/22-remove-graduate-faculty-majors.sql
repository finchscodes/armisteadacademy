-- Run this in Supabase's SQL Editor after 21-profiles-and-relations.sql.
-- Removes "Graduate" and "Faculty" as selectable/assignable majors —
-- graduation is now purely a computed year label (still shows "Graduate"
-- once a character crosses the lesson threshold), and Faculty is a job
-- (already exists in the Job List), not a major. Any character currently
-- set to either of these majors falls back to Undecided automatically —
-- nothing errors, nothing is lost, just double-check /admin/users afterward
-- if you want to hand-pick a more accurate major for anyone affected.
