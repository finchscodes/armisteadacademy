-- Run this in Supabase's SQL Editor after 31-reorder-halls.sql.
-- Adds class enrollment (students must enroll before lessons open up, and
-- only enrolled classes feed a character's grading bin) and switches lesson
-- rewards from a min/max range to a single flat reward with fixed tier
-- multipliers (Perfect 100%, Excellent 80%, Good 50%, Needs Improvement
-- 30%, Failing 0%, rounded up).
--
-- Existing lessons keep their old reward_max as the new flat reward — no
-- lesson silently resets to a bare default. Anyone who already submitted
-- homework for a class is auto-enrolled in it, so the new enrollment
-- requirement doesn't retroactively lock out someone already partway
-- through a class.
