-- Run this in Supabase's SQL Editor if privacy_policy.content is empty or
-- the row is missing. Safe to run regardless — it only touches the row if
-- content is currently blank; it will not overwrite anything you've
-- since written and saved through /admin/privacy.
...
WHERE privacy_policy.content = '' OR privacy_policy.content IS NULL;
