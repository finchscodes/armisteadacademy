...
If you already ran everything through `14`, you need two new files, **run as
separate queries, in order**:
1. `15-article-boards-and-permissions.sql` — adds the "article" board kind
   and the permission table for article-board posting rights. Run this one
   by itself first.
2. `16-notice-and-community-boards.sql` — adds the actual Notice Board and
   Community Board under Dormitories. Run this one second, as its own query.
...
| `03`–`14` | (historical) incremental updates, all folded into `01` |
| `15-article-boards-and-permissions.sql` | Article board kind + permissions table |
| `16-notice-and-community-boards.sql` | Adds Notice Board and Community Board |
