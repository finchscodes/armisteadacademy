...
Run, in order:
1. `01-schema.sql` — full schema, includes everything through migration `63`
...
Skip everything else — `03`–`62` are incremental updates already folded into
`01` for a fresh install (`03`, `04`, `07`, `10`, `12` into the original
schema snapshot; `48`–`63` appended on top of that). Running any of them
after `01` will error (trying to add/drop things that already don't exist
that way).

## Already had this running before this update

If you already ran everything through `62`, you only need the one new file:
1. `63-post-dice-rolls.sql`

## File reference

| File | What it does |
|---|---|
| `01-schema.sql` | Full schema through migration `63` (fresh installs only) |
