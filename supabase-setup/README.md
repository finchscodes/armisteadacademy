...
If you already ran everything through `13`, you need one new file:
1. `14-multi-job-support.sql` — jobs move to a proper multi-job table so a
   character can hold several jobs at once. Any existing single job is
   carried over automatically — nothing is lost.
...
| `03`–`13` | (historical) incremental updates, all folded into `01` |
| `14-multi-job-support.sql` | Characters can hold multiple jobs at once |
