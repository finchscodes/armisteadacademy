...
If you already ran everything through `16`, you need two new files, in order:
1. `17-extra-article-job.sql` — lets an article board auto-permit one extra
   job (used for Writers on Armistead Weekly / Inside Ploy).
2. `18-more-boards.sql` — adds Armistead Weekly, Inside Ploy, The Past,
...
| `03`–`16` | (historical) incremental updates, all folded into `01` |
| `17-extra-article-job.sql` | Extra allowed job per article board |
| `18-more-boards.sql` | Armistead Weekly, Inside Ploy, The Past, Missions, Elsewhere |
